  ...
  for i=1, #songs do
    ...
    if c then
      ...
      elseif string.match(c, "%d") then 
        -- Leave start as the first encountered number.
        -- (groups all numbers together.)
        best = c
      elseif best < c then
        -- When encounters first non-digit character, 
        -- sets the block title of numbers to #.
        if string.match(best, "%d") then
          songs[start]["newblk"], start, best = "\\#", i, c
        else
